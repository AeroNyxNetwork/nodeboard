 ...
 *     buckets, sequence numbers, and stable reason buckets. They must never
 *     expose full node IDs, endpoint URLs, route IDs, encrypted payloads,
 *     client IPs, DNS contents, Memory Chain plaintext, social graph edges,
 *     private keys, voucher secrets, or wallet-level traffic.
 *   v1.5.40 - Added Rust PeerStore network_story status so nodeboard can
 *     render product-facing protocol discovery readiness: verified peer view,
 *     routeable chat relay candidates, future onion-shaped path readiness,
 *     and restart recovery. This is aggregate node-control-plane metadata
 *     only and must not expose full node IDs, endpoints, route IDs, encrypted
 *     payloads, receiver identities, client IPs, DNS, destinations, Memory
 *     Chain plaintext, or wallet-level traffic.
...
export interface DiscoveryPeerStoreStatus {
  ...
  /**
   * Privacy-safe peer lifecycle rows generated by Rust PeerStore.
   *
   * Nodeboard may display these as discovery motion evidence, but must keep
   * them at the prefix/bucket level. Do not expand these rows into endpoints,
   * full public keys, route IDs, payloads, user relationships, DNS contents,
   * Memory Chain plaintext, or wallet-level traffic.
   */
  recent_peer_events?: DiscoveryPeerEvent[];
  ...
  /**
   * Product-facing aggregate discovery readiness story generated by Rust.
   *
   * This is safe for nodeboard, App, and website surfaces because it contains
   * only aggregate peer counts, route-readiness booleans, and restart recovery
   * state. It must never be expanded to expose full node IDs, endpoints, route
   * IDs, encrypted payloads, receiver identities, client IPs, destinations,
   * DNS contents, Memory Chain plaintext, voucher secrets, private keys, or
   * wallet-level traffic.
   */
...
export interface DiscoveryNetworkStoryStatus {
  generated_at: number;
  status: 'disabled' | 'discovering' | 'peer_view_ready' | 'relay_ready' | 'onion_ready' | 'attention' | string;
  headline: string;
  detail: string;
  discovered_nodes: number;
  valid_nodes: number;
  chat_relay_nodes: number;
  onion_middle_nodes: number;
...
export interface DiscoveryPeerEvent {
  at: number;
  event: string;
  outcome: string;
  source: string;
